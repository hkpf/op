...
fit <- lm(mpg ~ cyl + disp + hp +  wt + qsec, data = mtcars)
pr <- function(data.frame){
    predict(fit, newdata = data.frame)
}
api.test5 <- publishService(
    "test5123",
    code = pr,
    model = fit,
    inputs = list(data.frame = "data.frame"),
    outputs = list(answer = "numeric"),
    v = "v1.0.0"
)
pr(mtcars[1,])
result <- api.test5$pr(mtcars[1,]) 
str(result)
...
## need Input transformation: c(3,4,4) -> "c(3,3,3)" -> "3,3,3" TO DO: continue here!!! maybe give function as a string !!! see 
## https://docs.microsoft.com/de-de/machine-learning-server/operationalize/how-to-deploy-web-service-publish-manage-in-r#workflow
...
##  model with 784 variables, not hardcoded: paste together prediction function with variables as input, not needed for our case ####
...
## our model with input = data.frame, without rx-functions:
################################
# IMPORTANT:                   #
################################
# always when publishing an API need to first install the random forest package remotely, namely by hardcoding it into the prediction function.
# Afterwards, one can remove the install- and library-commands from the prediction function. 
...
predictsmall <- function(dataframe){
    ...
    #library(randomForest)
    predict(modelsmall, newdata = dataframe, type = "response") # also not working with predict.randomForest
}

apismalldf <- publishService(
    "modelsmalldf",
    code = predictsmall,
    model = modelsmall,
...
deleteService("modelsmalldf", "v1.0.0")

dtest <- readRDS("mnist_dataframes/mnist_test_dataframe.rds")
predictsmall(dtest[1,-785]) # lokal klappts
result <- apismalldf$predictsmall(dtest[1,-785]) 
str(result)
result$errorMessage
#"Error in UseMethod(\"predict\") : \n  
# no applicable method for 'predict' applied to an object of class \"randomForest\"\no answer could not be returned.\n"



# using session = TRUE starts a remote R session (prompt REMOTE> in console), sth. the missing randomForest package can be installed. 
# then do the following in remote console:
# REMOTE> install.packages("randomForest")
# REMOTE> library(randomForest)
# but doesn't help, since the packages are installed into the wrong library: "/usr/lib64/microsoft-r/3.3/lib64/R/library"
# but the api is connected to the "/usr/lib64/microsoft-r/rserver/o16n/9.1.0/rserve/workdir/Rserv9.1.0/conn9119" library
# also not working with predict.randomForest

# switch between local and remote R session: 
# > resume()
# REMOTE> pause()
